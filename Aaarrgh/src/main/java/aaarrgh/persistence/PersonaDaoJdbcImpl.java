...
public class PersonaDaoJdbcImpl implements PersonaDao {
	...
	public void insert(Persona persona) throws PersistenceException {
		...
		try {
			tx.begin();
			String query = "insert into persona (id, nombre, apellido, edad, pass) values (?, ?, ?, ?,?)";
			...
			statement.setString(5, persona.getPass());
	...
	public void update(Persona persona) throws PersistenceException {
		try {
			String query = "update persona set nombre = ?, apellido = ?, edad = ?, pass = ? where id = ?";
	...
	/* -------------Aca se busca por Name, se busca en la bbdd-------------*/
	 @Override
	public Persona findByName(String nombre) throws PersistenceException {
		if (nombre == null) {
			throw new IllegalArgumentException(
					"El nombre de persona no debe ser nulo");
		}
		Persona persona = null;
		try {
			Connection c = ConnectionProvider.getInstance().getConnection();
...
/*----------Termina lo que hice----------*/
	...
	@Override
	public Persona findByNameAndPass(String nombre, String pass)
		...
		try {
			Connection c = ConnectionProvider.getInstance().getConnection();
			String query = "select * from persona where nombre = ? and pass = ?";
			PreparedStatement statement = c.prepareStatement(query);
			statement.setString(1, nombre);
